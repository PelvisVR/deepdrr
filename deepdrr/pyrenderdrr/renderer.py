...
class Renderer(object):
    ...
    def render(self, scene, flags, seg_node_map=None, drr_mode=DRRMode.NONE, zfar=0, mat=None, mat_idx=None, layer_id=None):
        self._update_context(scene, flags)

        if drr_mode == DRRMode.DIST:
            for i in range(self.num_peel_passes):
                retval = self._forward_pass(scene, flags, seg_node_map=seg_node_map, drr_mode=drr_mode, zfar=zfar, peelnum=i, mat=mat, mat_idx=mat_idx, layer_id=layer_id)
        elif drr_mode == DRRMode.MESH_SUB:
            for i in range(self.num_peel_passes*2):
                retval = self._forward_pass(scene, flags, seg_node_map=seg_node_map, drr_mode=drr_mode, zfar=zfar, peelnum=i, mat=mat, mat_idx=mat_idx, layer_id=layer_id)
        elif drr_mode == DRRMode.DENSITY:
            retval = self._forward_pass(scene, flags, seg_node_map=seg_node_map, drr_mode=drr_mode, zfar=zfar, peelnum=0, mat=mat, mat_idx=mat_idx, layer_id=layer_id)
        else:
    ...
    def _forward_pass(self, scene, flags, seg_node_map=None, drr_mode=DRRMode.NONE, zfar=0, peelnum=0, mat=None, mat_idx=None, layer_id=None):
    ...
    def _configure_forward_pass_viewport(self, flags, drr_mode=DRRMode.NONE, peelnum=0, mat_idx=None, layer_id=None):
